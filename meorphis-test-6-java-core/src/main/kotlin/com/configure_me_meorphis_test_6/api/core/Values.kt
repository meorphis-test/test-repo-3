...
sealed class JsonField<out T: Any> {
    ...
    fun asStringOrThrow(): String =
        when (this) {
            is JsonString -> value
            else -> throw MeorphisTest6InvalidDataException("Value is not a string")
        }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw MeorphisTest6InvalidDataException("'${name}' is not set")
            is JsonNull -> throw MeorphisTest6InvalidDataException("'${name}' is null")
            else -> throw MeorphisTest6InvalidDataException("'${name}' is invalid, received ${this}")
        }
